...
describe("DecisionLog API", () => {
  ...
  it("permite administrador atualizar foto pessoal e foto da empresa", async () => {
    const avatarUrl = "data:image/png;base64,aaaaaaaaaaaa";
    const companyLogoUrl = "data:image/png;base64,bbbbbbbbbbbb";
    ...
    mocks.prisma.user.update.mockResolvedValue({
      id: "user-1",
      companyId,
      company: { id: companyId, name: "DecisionLog", slug: "decisionlog", logoUrl: companyLogoUrl },
      name: "Jamille Admin",
      email: "admin@decisionlog.local",
      avatarUrl,
      role: "admin",
  ...
  it("bloqueia gestor de alterar foto da empresa", async () => {
    mocks.prisma.user.findUnique.mockResolvedValue({
      id: "user-1",
      companyId,
      name: "Gestor",
    ...
    const response = await request(app)
      .patch("/users/me")
      .set("Authorization", `Bearer ${makeToken("manager")}`)
      .send({ companyLogoUrl: "data:image/png;base64,bbbbbbbbbbbb" });
